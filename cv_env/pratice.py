        ...
        if isDragging:                  # 드래그 중지
            ...
            if w > 0 and h > 0:         # 폭과 높이가 양수이면 드래그 방향이 옳음 ---⑤
                ...
                for contour in contours:
                    ...
                    #입력외곽선,근사화정밀도를 결정하는 값으로 작을수록 더 정확한 근사화가 이루어짐, 외곽선이 닫혀있는 지를 나타내는 불리언 값

                    cv2.drawContours(contour_image,[hull],-1,(0,255,0),thickness=2)
                    # 최소한의 사각형 표시(노란색)
                    rect = cv2.minAreaRect(contour)
                    box = cv2.boxPoints(rect)   # 중심점과 각도를 4개의 꼭지점 좌표로 변환
                    box = np.intp(box)          # 정수로 변환
                    cv2.drawContours(contour_image, [box], -1, (0,255,255), thickness=2)
